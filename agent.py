...
# ==========================================
# 5. 主程序入口 (GitHub Actions 适配版)
# ==========================================
if __name__ == "__main__":
    if register_node():
        print("▶️ 打卡成功，开始执行 GitHub Actions 巡逻任务...")
        
        # 设定每次唤醒最多处理 3 个任务，防止单次 Action 运行太久被强杀
        tasks_handled = 0
        max_tasks_per_run = 3
        
        while tasks_handled < max_tasks_per_run:
            try:
                if fetch_and_solve_task():
                    tasks_handled += 1
                    time.sleep(3) # 做完一单稍微歇一下继续接
                else:
                    print("📭 当前大厅没有能接的任务了，收工下线！")
                    break # 没任务了直接退出，节省 GitHub 算力
            except Exception as e:
                print(f"⚠️ 发生异常: {e}")
                break
                
        print(f"🛑 本次巡逻结束，共成功处理 {tasks_handled} 个任务。")
    else:
